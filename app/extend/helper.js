 ...
 * @returns {string}
 */
exports.randomSMSCode = (len) => {
  let nums = "0123456789".split('').sort(() => {
    return Math.random() > .5 ? -1 : 1;
  });
  return nums.splice(0, Math.min(len, 9)).join('');
}
...
exports.createDefaultHeadImg = () => {
  ...
  let rand = Math.floor(Math.random() * headIMgs.length);
  return `/images/${headIMgs[rand]}`
}
